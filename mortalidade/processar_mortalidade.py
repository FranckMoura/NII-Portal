...
if dados_consolidados:
    ...
    try:
        # Tenta apagar dados antigos (Se der erro de permissão, ele avisa)
        try:
            supabase.table('mortalidade').delete().neq("id", 0).execute()
        except:
            print("   ⚠️ Aviso: Não foi possível limpar a tabela antes (permissão). Tentando apenas inserir...")
        ...
        print(f"\n❌ ERRO NO ENVIO: {e}")
        print("Dica: Verifique se você criou a tabela 'mortalidade' no site do Supabase.")
else:
