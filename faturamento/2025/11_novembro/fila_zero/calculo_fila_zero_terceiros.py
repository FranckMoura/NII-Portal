...
def gerar_html_terceiros(df, nome_arquivo, competencia_label):
    ...
    <head>
        ...
        <title>Fila Zero Terceiros - {competencia_label}</title>
        ...
        <div class='header-bg p-8 shadow-lg mb-8'>
            <div class='max-w-7xl mx-auto'>
                <div class="flex items-center gap-4">
                    ...
                    <div><h1 class='text-3xl font-bold'>Fila Zero - Terceiros/SADT</h1><p class='text-blue-100'>Competência: {competencia_label} | Gerado em: {datetime.now().strftime("%d/%m/%Y")}</p></div>
                </div>
            </div>
        </div>
...
if __name__ == "__main__":
    if ARQUIVO_ENTRADA:
        ...
        if not df_dados.empty:
            ...
            nome_html = os.path.join(PASTA_SCRIPT, f"relatorio_fila_zero_terceiros_{comp_sufixo}.html")
