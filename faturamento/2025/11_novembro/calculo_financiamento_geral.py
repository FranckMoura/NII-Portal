# ==============================================================================
# SISTEMA DE REPASSES - PRODUÇÃO POR FINANCIAMENTO GERAL (V1.0)
# Autor: Franck Moura (Via NII Automation)
# Data: 29/12/2025
# Descrição: Processa R_PROD_FINANCIAMENTO (Geral).
#            Agrupa valores por BLOCO DE FINANCIAMENTO (MAC, FAEC, Estratégico).
# ==============================================================================
...
print(f"--- Processando Produção por Financiamento (Geral) ---")

# Busca: Arquivos que TEM "FINANCIAMENTO" mas NÃO TEM "FILAZERO"
arquivos_todos = glob.glob(os.path.join(PASTA_SCRIPT, 'R_PROD_FINANCIAMENTO*.pdf'))
pdf_financiamento = [f for f in arquivos_todos if "FILAZERO" not in f.upper()]
    ...
    with pdfplumber.open(caminho) as pdf:
        for page in pdf.pages:
            ...
            for line in text.split('\n'):
                ...
                # Identifica o Grupo de Financiamento (Ex: "Financiamento: 04 - FAEC")
                if "Financiamento:" in line or "FINANCIAMENTO:" in line:
                    partes = line.split(":")
                    if len(partes) > 1:
                        grupo_atual = partes[1].strip().upper()
                    continue

                # Identifica Procedimento (Código no início)
                ...
                if match_cod:
                    ...
                    if valores:
                        try:
                            # O último valor é o Total Aprovado
                            val_str = valores[-1].replace('.', '').replace(',', '.')
                            valor = float(val_str)
                            
                            # Tenta pegar quantidade (numero inteiro antes dos valores monetarios)
                            qtd = 1
                            nums = re.findall(r'\s(\d+)\s', line)
                            # Pega o penultimo numero (heuristicamente qtd) se houver varios
                            if nums: qtd = int(nums[-1])

                            dados.append({
                                'Financiamento': grupo_atual,
                                'Codigo': codigo,
                                'Descricao': descricao,
                                'Qtd': qtd,
                                'Valor': valor
...
if __name__ == "__main__":
    if ARQUIVO_ENTRADA:
        ...
        if not df_dados.empty:
            ...
            nome_html = os.path.join(PASTA_SCRIPT, f"relatorio_financiamento_geral_{comp_sufixo}.html")
            total = gerar_html(df_dados, nome_html, comp_label, "Produção por Financiamento (Geral)")
            caminho_web = os.path.relpath(nome_html, r"C:\Users\DELL\OneDrive\NII-Portal-1").replace("\\", "/")
            reg = { "titulo": f"Financiamento Geral - {comp_label}", "competencia": comp_label, "data_geracao": datetime.now().strftime("%d/%m/%Y %H:%M"), "valor_total": f"R$ {total:,.2f}", "arquivo": caminho_web }
            atualizar_portal(reg)
        else: print("⚠️ Nenhum dado encontrado no PDF.")
    else: print("❌ Arquivo 'R_PROD_FINANCIAMENTO' (Geral) não encontrado.")